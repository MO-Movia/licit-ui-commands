...
describe('updateIndentLevel', () => {
    ...
    it('should be check the condition inside setListNodeIndent function() !listItem',()=>{
        const state = {} as unknown as EditorState;
        const tr = {doc:dummyDoc,selection:{from:1,to:2},getMeta: () => { return 'dryrun'; }} as unknown as Transform;
        const sc = {nodes:{'paragraph':PARAGRAPH, 'heading':HEADING,'blockquote':BLOCKQUOTE}} as unknown as Schema;
        const view = {} as unknown as EditorView;
    ...
    it('should be return tr inside setListNodeIndent function()',()=>{
        ...
        const tr = {doc: dummyDoc, selection: {from: 0, to: doc.content.size}, getMeta: () => 'dryrun', setNodeMarkup: () => true } as unknown as Transform;

        const sc = {nodes:{'paragraph':PARAGRAPH, 'heading':undefined,'blockquote':undefined,'list_item':LIST_ITEM}} as unknown as Schema;
        const view = {} as unknown as EditorView;
        jest.spyOn(isListNode, 'default').mockReturnValue(true) as unknown as Node;
        jest.spyOn(consolidateListNodes, 'default').mockReturnValue( tr as unknown as Transform );
        const test = updateIndentLevel(state,tr,sc,5,view);
    ...
    it('should be check the condition indentNew === listNode.attrs.indent',()=>{
        ...
        // Create a dummy document using the defined schema
        const dummyDoc = mySchema.node('doc', null, [
            mySchema.node('heading', { indent: 5 }, [
                mySchema.text('Heading 1'),
            ]),
        ...
        const tr = {doc:dummyDoc,selection:{from:0,to: doc.content.size},getMeta: () => { return 'dryrun'; }, setNodeMarkup:()=>{return true;}} as unknown as Transform;
        const sc = {nodes:{'paragraph':PARAGRAPH, 'heading':undefined,'blockquote':undefined,'list_item':LIST_ITEM}} as unknown as Schema;
        const view = {} as unknown as EditorView;
        jest.spyOn(isListNode, 'default').mockReturnValue(true) as unknown as Node;
        jest.spyOn(consolidateListNodes, 'default').mockReturnValue( tr as unknown as Transform );
...
xit('',()=>{
    ...
    jest.spyOn(clamp, 'default').mockReturnValue(5);
    const sc = {nodes:{'list_item':LIST_ITEM}} as unknown as Schema;
    const tr = {selection:{from:1,to:1},doc:dummyDoc} as unknown as Transform;
    const state = {} as unknown as EditorState;
    const test = setListNodeIndent(state,tr,sc,2,22);
...
it('should be check the condition !node inside the function setNodeIndentMarkup()',()=>{
    const state = {}as unknown as EditorState;
    const tr = {doc:{nodeAt:()=>{return false;}}} as unknown as Transform;
    const test = setNodeIndentMarkup(state,tr,9,7);
